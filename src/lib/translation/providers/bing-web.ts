...
function chunkMarkedTokens(tokens: MarkedToken[]): MarkedChunk[] {
  ...
  for (const token of tokens) {
    ...
    while (start < token.text.length) {
      ...
      start = end;
      if (
        current.text.length === MAX_TEXT_LENGTH ||
...
export function parseBingCredentials(html: string): BingCredentials {
  ...
  const params = html.match(
    /params_(?:AbusePreventionHelper|RichTranslateHelper)\s*=\s*\[\s*(\d+)\s*,\s*["']([^"']+)/,
  );
  if (!iid || !ig || !params) {
    throw new Error("Unable to obtain Bing translation credentials.");
  }
...
export class BingWebProvider implements TranslationProvider {
  ...
  readonly label = "Bing Translator";
  ...
  private async getCredentials(signal?: AbortSignal) {
    ...
    if (!response.ok) {
      throw new Error(
        `The upstream service rejected the Bing credential request (HTTP ${response.status}).`,
      );
    }
  ...
  async translate(input: ProviderInput, signal?: AbortSignal) {
    ...
    const translateChunk = async (text: string) => {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `The upstream service rejected the Bing translation request (HTTP ${response.status}).`,
        );
      }
      ...
      if (!responseText) {
        this.credentials = undefined;
        throw new Error(
          "Bing returned no translation, possibly due to regional restrictions or rate limiting.",
        );
      }
      ...
      if (!translatedText) {
        throw new Error("Bing returned an unrecognized response.");
      }
      ...
      if (!token.translatable) translatedChunks.push(token.text);
        ...
        while (start < token.text.length) {
          ...
          translatedChunks.push(
            await translateChunk(token.text.slice(start, end)),
          );
          start = end;
        }
      }
